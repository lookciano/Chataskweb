...
async function createTaskImage(task: ShareableTask) {
  ...
  for (const word of description.split(/\s+/)) {
    ...
    if (context.measureText(candidate).width > width - padding * 2 && current) {
      lines.push(current);
      current = word;
  ...
  canvas.width = width;
  canvas.height = 350 + visibleLines.length * lineHeight;
  ...
  context.fillText("Chat Task", padding, 92);
  context.font = "bold 36px Arial";
  context.fillText(title, padding, 160);
...
export function TaskShareButton({ task }: { task: ShareableTask }) {
  ...
  return (
    <button type="button" className="p-1.5 text-slate-400 hover:text-teal-700 hover:bg-teal-50 rounded-lg transition-colors" title="Compartilhar tarefa como imagem" aria-label="Compartilhar tarefa como imagem" onClick={(event) => { event.stopPropagation(); void handleShare(); }}>
